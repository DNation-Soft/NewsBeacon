...
function get_theme_settings(){
    ...
    $table = DB()->table('nb_theme_settings');
...
{
    $table = DB()->table('nb_product_attribute');
...
{
    $table = DB()->table('nb_product_to_category');
    $count = $table->join('nb_products', 'nb_products.product_id = nb_product_to_category.product_id')->where('nb_product_to_category.category_id', $category_id)->where('nb_products.status','Active')->countAllResults();
    return $count;
}
...
{
    $table = DB()->table('nb_product_feedback');
...
function order_email_template($orderId)
{
    ...
    $tableItem = DB()->table('nb_order_item');
    ...
    $paymentMet = get_data_by_id('name','nb_payment_method','payment_method_id',$val->payment_method);
    ...
    foreach ($item as $row) {
        $proName = get_data_by_id('name', 'nb_products', 'product_id', $row->product_id);
        $model = get_data_by_id('model', 'nb_products', 'product_id', $row->product_id);
        $image = get_data_by_id('image', 'nb_products', 'product_id', $row->product_id);
...
{
    $table = DB()->table('nb_order_history');
    $order = $table->where('order_id', $order_id)->get()->getLastRow();
    return get_data_by_id('name', 'nb_order_status', 'order_status_id', $order->order_status_id);
}
...
{
    $qtyCheck = get_data_by_id('quantity', 'nb_products', 'product_id', $product_id);
    $optionCheck = is_exists('nb_product_option', 'product_id', $product_id);
    $btn = '';
    if (!empty($qtyCheck)) {
        if ($optionCheck == true) {
...
{
    $table = DB()->table('nb_order_option');
...
{
    $table = DB()->table('nb_module_settings');
...
{
    $rowApi = get_all_row_data_by_id('nb_payment_settings', 'label', 'api_url');
    $api_username = get_all_row_data_by_id('nb_payment_settings', 'label', 'api_username');
    $api_password = get_all_row_data_by_id('nb_payment_settings', 'label', 'api_password');
    $api_signature = get_all_row_data_by_id('nb_payment_settings', 'label', 'api_signature');
...
{
    $table = DB()->table('nb_products');
    $table->join('nb_product_to_category', 'nb_product_to_category.product_id = nb_products.product_id')->where('nb_products.status', 'Active');
    $result = $table->where('nb_product_to_category.category_id', $category_id)->orderBy('nb_products.product_id','DESC')->limit(20)->get()->getResult();
...
function display_category_with_parent($cate_id)
{
    ...
    if (!empty($cate_id)) {
        ...
        for ($i=0; $i<=$totalParent; $i++) {
            ...
            $cat = $table->where('news_cat_id', $cate_id)->get()->getRow();
...
function top_menu()
{
    ...
    foreach ($query as $val) {
        $url = base_url('/' . urlencode($val->slug));
        $view .= '<li class="nav-item  pd-10"><a class="nav-link m-link" aria-current="page" href="' . $url . '" >' . $val->category_name . '</a></li>';
    }
    return $view;
}