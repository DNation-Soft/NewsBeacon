...
class Products extends BaseController {
    ...
    public function search(){
        ...
        if ($shortBy == 'price_asc'){
            $shortBy = "`nb_products.price` ASC";
        }elseif($shortBy == 'price_desc'){
            $shortBy = "`nb_products.price` DESC";
        }else{
            $shortBy = "`nb_products.product_id` DESC";
        }
        ...
        if(empty($this->request->getGetPost('option'))) {
            ...
            foreach ($options as  $key => $valOp){
                $optId = get_data_by_id('option_id','nb_product_option','option_value_id',$valOp);
                $arr = $optId;
        ...
        if(empty($this->request->getGetPost('price'))) {
            ...
            $firstPrice = 'nb_products.price >= '.$price[0];
            $lastPrice = 'nb_products.price <= '.$price[1];
        }
        ...
        if (!empty($categoryId)) {
            if (empty($this->request->getGetPost('option'))) {
                ...
                $data['products'] = $this->$searchModel->where($where)->all_join()->having('COUNT(nb_products.product_id) >=', count(array_unique($countOption)))->orderBy($shortBy)->paginate($lemit);
            }
        }

        if (!empty($keyword)){
            if(empty($this->request->getGetPost('option'))) {
                $data['products'] = $this->$searchModel->where($where)->like('nb_products.name', $keyword)->query()->orderBy($shortBy)->paginate($lemit);
            }else{
                $data['products'] = $this->$searchModel->where($where)->like('nb_products.name', $keyword)->all_join()->orderBy($shortBy)->paginate($lemit);
            }
        }
        ...
        if (!empty($cat_id)) {
            ...
            if (empty($manufacturer)) {
                if(!empty($keyword)) {
                    $productsArr = $this->$searchModel->like('nb_products.name', $keyword)->query()->findAll();
                }
            }else{
                $productsArr = $this->$searchModel->where($allbrand)->like('nb_products.name', $keyword)->query()->findAll();
                //make search base query
                $productsBas = $this->$searchModel->like('nb_products.name', $keyword)->query()->findAll();
            }
        }
        ...
        $table = DB()->table('nb_product_category');
        ...
        $data['title'] = (!empty($cat_id))?get_data_by_id('category_name','nb_product_category','prod_cat_id',$cat_id):'Search';
