...
class Checkout extends BaseController
{
    ...
    public function coupon_action()
    {
        ...
        if (!empty($query)) {
            ...
            if ($query->for_subscribed_user == '1') {
                $isLoggedInCustomer = $this->session->isLoggedInCustomer;
                if (isset($isLoggedInCustomer) || $isLoggedInCustomer == TRUE) {
                    $checkSub = is_exists('nb_newsletter', 'customer_id', $this->session->cusUserId);
                    if ($checkSub == false) {
                        if (!empty($this->cart->contents())) {
                            $couponArray = array(
                                'coupon_discount' => $query->discount
    ...
    public function checkout_action() {
        ...
        if (!empty($this->cart->contents())) {
            ...
            $new_anb_create = $this->request->getPost('new_anb_create');
            ...
            if ($this->validation->run($data) == FALSE) {
                ...
                if ($data['payment_method'] == '8') {
                    $balCus = get_data_by_id('balance', 'nb_customer', 'customer_id', $this->session->cusUserId);
                    if ($balCus < $finalAmo) {
                ...
                if ($data['payment_method'] == '8') {
                    ...
                    $tableCusLedg = DB()->table('nb_customer_ledger');
                ...
                if ($data['payment_method'] == '7') {
                    ...
                    $tableCard = DB()->table('nb_order_card_details');
    ...
    public function payment_instruction()
    {
        ...
        $table = DB()->table('nb_payment_settings');
