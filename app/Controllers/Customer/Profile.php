...
class Profile extends BaseController
{
    ...
    public function update_action(){
        ...
        if ($this->validation->run($data) == FALSE) {
            ...
            if (!empty($data['current_password'])){
                $check = is_exists_double_condition('nb_customer','customer_id',$this->session->cusUserId,'password',SHA1($data['current_password']));
                if ($check == false){
            ...
            $check_address = is_exists('nb_address','customer_id',$this->session->cusUserId);
            if ($check_address == true){
                $tabAd = DB()->table('nb_address');
    ...
    public function newsletter_action(){
        $check = get_data_by_id('newsletter','nb_customer','customer_id',$this->session->cusUserId);

        if ($check == '0') {
            $email = get_data_by_id('email','nb_customer','customer_id',$this->session->cusUserId);
            $newData['customer_id'] = $this->session->cusUserId;
            $newData['email'] = $email;
            $newAd = DB()->table('nb_newsletter');
            $newAd->insert($newData);
